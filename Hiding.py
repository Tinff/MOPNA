...
def EMD_2006(image_array,secret_string,n,image_file_name=''):
    ...
    print('EMD_2006 pixels used: %d' % num_pixels_changed)
    #重组图像
    ...
    new_file = filepath + '\\Output\\' + originfilename + "_EMD_2006.png"
    img_out.save(new_file,'png') 

    return 0

# ALGORITHM: LWC_2007 方法
def LWC_2007(image_array,secret_string,n=2,image_file_name=''):
    #image_array:输入的一维图像数组
    #image_file_name:传入的图像文件名（带全路径）
    #分成2个像素一组
    n = 2
    ...
    print('LWC_2007 pixels used: %d' % num_pixels_changed)
    #重组图像
    img_out = img_out.reshape(256,256)
    img_out = Image.fromarray(img_out)
    #img_out.show()
    ...
    (filepath,tempfilename) = os.path.split(image_file_name)        
    (originfilename,extension) = os.path.splitext(tempfilename)
    new_file = filepath + '\\Output\\' + originfilename + "LWC_20076.png"
    img_out.save(new_file,'png') 

    return 0

# ALGORITHM: JY_2009 方法
def JY_2009(image_array,secret_string,n=1,k=2,image_file_name=''):
    #image_array:输入的一维图像数组
    #image_file_name:传入的图像文件名（带全路径）
    #n = 1 #此算法在一个像素中嵌入
    ...
    img_array_out = img_out.copy()
    psnr = PSNR(image_array,img_array_out)
    print('JY_2009 k=%d PSNR: %.2f' % (k,psnr))
    print('JY_2009 k=%d pixels used: %d' % (k,num_pixels_changed))
    #重组图像
    img_out = img_out.reshape(256,256)
    img_out = Image.fromarray(img_out)
    #img_out.show()
    ...
    (filepath,tempfilename) = os.path.split(image_file_name)        
    (originfilename,extension) = os.path.splitext(tempfilename)
    new_file = filepath + '\\Output\\' + originfilename + "_JY_2009_k_"+str(k)+".png"
    img_out.save(new_file,'png') 

    return 0

# ALGORITHM: GEMD_2013 方法
def GEMD_2013(image_array,secret_string,n,image_file_name=''):
    #image_array:输入的一维图像数组
    #image_file_name:传入的图像文件名（带全路径）
    #n为一组像素的数量
    ...
    print('GEMD_2013 pixels used: %d' % num_pixels_changed)
    #重组图像
    img_out = img_out.reshape(256,256)
    img_out = Image.fromarray(img_out)
    #img_out.show()
    ...
    (filepath,tempfilename) = os.path.split(image_file_name)        
    (originfilename,extension) = os.path.splitext(tempfilename)
    new_file = filepath + '\\Output\\' + originfilename + "_GEMD_2013.png"
    img_out.save(new_file,'png') 

    return 0

# ALGORITHM: KKWW_2016 方法
def KKWW_2016(image_array,secret_string,n=4,k=3,image_file_name=''):
    #image_array:输入的一维图像数组
    #image_file_name:传入的图像文件名（带全路径）
    #n为一组像素的数量
   ...
   #-----------------------------------------------------------------------------------
    ...
    img_array_out = img_out.copy()
    psnr = PSNR(image_array,img_array_out)
    print('KKWW_2016 k=%d PSNR: %.2f' % (k,psnr))
    print('KKWW_2016 k=%d pixels used: %d' % (k,num_pixels_changed))
    #重组图像
    img_out = img_out.reshape(256,256)
    img_out = Image.fromarray(img_out)
    #img_out.show()
    ...
    (filepath,tempfilename) = os.path.split(image_file_name)        
    (originfilename,extension) = os.path.splitext(tempfilename)
    new_file = filepath + '\\Output\\' + originfilename + "_KKWW_2016_k_"+str(k)+".png"
    img_out.save(new_file,'png') 

    return 0

# ALGORITHM: SB_2019 方法
def SB_2019(image_array,secret_string,n=1,k=3,image_file_name=''):
    #image_array:输入的一维图像数组
    #image_file_name:传入的图像文件名（带全路径）
    n = 1 #此算法在一个像素中嵌入
    num_pixel_groups = image_array.size
    ...
    img_array_out = img_out.copy()
    psnr = PSNR(image_array,img_array_out)
    print('SB_2019 k=%d PSNR: %.2f' % (k,psnr))
    print('SB_2019 k=%d pixels used: %d' % (k,num_pixels_changed))
    #重组图像
    img_out = img_out.reshape(256,256)
    img_out = Image.fromarray(img_out)
    #img_out.show()
    ...
    (filepath,tempfilename) = os.path.split(image_file_name)        
    (originfilename,extension) = os.path.splitext(tempfilename)
    new_file = filepath + '\\Output\\' + originfilename + "_SB_2019 k_" + str(k) + ".png"
    img_out.save(new_file,'png') 
...
def MOPNA(image_array,secret_string,n=2,k=3,image_file_name=''):
    ...
    diffarray = recover_d_array - secret_d_array
    assert(int((recover_d_array - secret_d_array).sum()) == 0)
    #-----------------------------------------------------------------------------------
    #输出图像
    img_out = embedded_pixels_group.flatten()
    ...
    img_array_out = img_out.copy()
    psnr = PSNR(image_array,img_array_out)
    print('MOPNA k=%d PSNR: %.2f' % (k,psnr))
    print('MOPNA k=%d pixels used: %d' % (k,num_pixels_changed))
    ...
    (filepath,tempfilename) = os.path.split(image_file_name)        
    (originfilename,extension) = os.path.splitext(tempfilename)
    new_file = filepath + '\\Output\\' + originfilename + "_MOPNA_k_" + str(k) + ".png"
    img_out.save(new_file,'png') 
    #-----------------------------------------------------------------------------------
    return
...
for file in os.listdir(path):  
    ...
    if os.path.isfile(file_path):  
        ...
        #调用函数
        #EMD_2006(img_array3,s_data,4,file_path)
        #LWC_2007(img_array3,s_data,2,file_path)

        #JY_2009(img_array3,s_data,1,1,file_path)
        #JY_2009(img_array3,s_data,1,2,file_path)

        #GEMD_2013(img_array3,s_data,4,file_path)

        #KKWW_2016(img_array3,s_data,4,2,file_path)
        #KKWW_2016(img_array3,s_data,4,3,file_path)

        #SB_2019(img_array3,s_data,1,2,file_path)
        #SB_2019(img_array3,s_data,1,3,file_path)
